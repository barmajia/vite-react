...
// Target languages with their names
const languages = {
  de: { name: 'German', flag: '🇩🇪' },
  ...
  ar: { name: 'Arabic', flag: '🇸🇦' } // Already has health section
};

// Professional translations for key sections (manually curated)
const translations = {
  health: {
    de: {
  ...
  categories: {
    ...
    it: {
      ...
      sports: "Sport e attività all'aperto",
      beauty: "Bellezza e cura personale",
      books: "Libri e media",
    ...
    tr: {
      ...
      sports: "Spor ve açık hava",
      beauty: "Güzellik ve kişisel bakım",
      books: "Kitap ve medya",
...
function generateTranslationFile(langCode, langInfo) {
  ...
  try {
    existingTranslations = require(existingPath);
  ...
  if (translations.health[langCode]) {
    updatedTranslations.health = mergeTranslations(
      updatedTranslations.health || {},
      translations.health[langCode]
    );
  }
  
  if (translations.categories[langCode]) {
    updatedTranslations.categories = mergeTranslations(
      updatedTranslations.categories || {},
      translations.categories[langCode]
    );
  }
  
  // Write updated file
  const outputPath = existingPath;
  fs.writeFileSync(outputPath, JSON.stringify(updatedTranslations, null, 2), 'utf-8');
...
function main() {
  ...
  for (const [langCode, langInfo] of Object.entries(languages)) {
    if (langCode === 'ar') {
      console.log(`⊘ Skipping ${langInfo.name} ${langInfo.flag} - already has health section`);
      continue;
    }
