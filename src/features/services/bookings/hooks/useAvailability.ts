 ...
 */
export const useAvailability = (providerId: string | undefined, date: Date | undefined) => {
  return useQuery({
    queryKey: ['availability', providerId, date ? format(date, 'yyyy-MM-dd') : null],
    queryFn: async (): Promise<AvailabilityData | null> => {
      ...
      if (availability && availability.length > 0) {
        availability.forEach((avail) => {
          ...
          for (let hour = startHour; hour < endHour; hour++) {
            for (let min = 0; min < 60; min += duration) {
              ...
              const isBooked = bookings?.some(b => b.booking_time === time);
        ...
        bookings?.forEach(booking => {
          const slot = slots.find(s => s.time === booking.booking_time);
