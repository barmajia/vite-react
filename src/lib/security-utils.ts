...
import { SECURITY_CONSTANTS, SecurityEventType, SecurityLevel, AuditLogEntry, SecurityContext } from './security-constants';
 ...
 */
export function sanitizeXSS(input: string, options: { allowBasicHTML?: boolean } = {}): string {
  if (!input) return '';
  ...
  if (!options.allowBasicHTML) {
    sanitized = sanitized.replace(/<\s*(script|iframe|object|embed|frame|frameset|applet|meta|link|style|form|input|button|textarea|select)[^>]*>/gi, '');
  }
...
export function detectSQLInjection(input: string): boolean {
  ...
  const matches = SQL_INJECTION_PATTERNS.filter((pattern) => pattern.test(input));
...
export function getSecurityHeaders(): Record<string, string> {
  return {
    ...
    'X-Content-Type-Options': SECURITY_CONSTANTS.SECURITY_HEADERS.X_CONTENT_TYPE_OPTIONS,
    ...
    'Permissions-Policy': SECURITY_CONSTANTS.SECURITY_HEADERS.PERMISSIONS_POLICY,
    'Strict-Transport-Security': SECURITY_CONSTANTS.SECURITY_HEADERS.STRICT_TRANSPORT_SECURITY,
  };
}
...
export function validateEmail(email: string): boolean {
  if (!email || email.length > SECURITY_CONSTANTS.INPUT_VALIDATION.MAX_EMAIL_LENGTH) {
    return false;
  }
 ...
 */
export function maskSensitiveData(data: string, type: 'email' | 'phone' | 'credit_card' | 'ssn'): string {
  switch (type) {
    case 'email':
