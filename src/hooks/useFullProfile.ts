    ...
    queryFn: async () => {
      ...
      if (userError || !coreUser) {
        ...
        return {
          ...
          addresses: [],
          stats: {
            orders: { totalOrders: 0, pendingOrders: 0, completedOrders: 0, totalSpent: 0, totalEarned: 0 },
      ...
      if (userProfile.account_type === 'seller' || userProfile.account_type === 'factory') {
      ...
      if (userProfile.account_type === 'middleman') {
        const [{ data: middleman, error: middlemanError }, { data: business }] = await Promise.all([
          supabase.from('middleman_profiles').select('*').eq('user_id', targetUserId).maybeSingle(),
          supabase.from('business_profiles').select('*').eq('user_id', targetUserId).maybeSingle(),
        ]);
        middlemanData = middleman;
        businessData = business;
        if (middlemanError && middlemanError.code !== 'PGRST116') {
          console.warn('Could not fetch middleman data:', middlemanError.message);
        }
      }
      ...
      const [ordersRes, salesRes, notificationsRes, wishlistRes] = await Promise.all([
        supabase.from('orders').select('status, total').eq('user_id', targetUserId),
        supabase.from('orders').select('total').eq('seller_id', targetUserId),
        supabase.from('notifications').select('is_read').eq('user_id', targetUserId),
      ...
      return {
        ...
        stats: {
          orders: {
            ...
            totalEarned: salesData.reduce((sum: number, o: any) => sum + (o.total || 0), 0),
          },
          notifications: {
            total: notificationsRes.data?.length || 0,
            unread: notificationsRes.data?.filter((n: any) => !n.is_read).length || 0,
          },
          wishlist: {
