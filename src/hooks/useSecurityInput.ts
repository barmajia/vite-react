...
export function useSecurityInput(options: UseSecurityInputOptions = {}) {
  ...
  const validate = useCallback((inputValue: string): SecurityValidationResult => {
    const errors: string[] = [];
  ...
  }, [type, validateXSS, validateSQLInjection, validatePathTraversal, maxLength, minLength, required, pattern, sanitize]);
...
export function useSecureFileUpload(options: {
  ...
  const {
    ...
    allowedTypes = [...SECURITY_CONSTANTS.FILE_UPLOAD_CONFIG.ALLOWED_IMAGE_TYPES, ...SECURITY_CONSTANTS.FILE_UPLOAD_CONFIG.ALLOWED_DOCUMENT_TYPES],
  ...
  const handleFileChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      setValidation({ isValid: result.isValid, errors: result.errors, warnings: result.warnings });
