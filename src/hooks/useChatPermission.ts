...
import { TradingAccountType, ChatPermissionResult, ProductWithChatPermission } from '../types/trading-chat';
import { ALLOWED_CONVERSATION_FLOWS, TRADING_ACCOUNT_CONFIG } from '../lib/tradingConfig';
...
export const useChatPermission = (
  ...
  const [permission, setPermission] = useState<ChatPermissionResult>({ allowed: false });
  const [product, setProduct] = useState<ProductWithChatPermission | null>(null);
  ...
  const checkPermission = async () => {
    ...
    if (productId && TRADING_ACCOUNT_CONFIG[currentUserType].requiresProductPermission) {
    ...
    setPermission({
      allowed: true,
      requiresProductPermission: TRADING_ACCOUNT_CONFIG[currentUserType].requiresProductPermission,
