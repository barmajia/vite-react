...
export class SecurityBoundary extends Component<SecurityBoundaryProps, SecurityBoundaryState> {
  ...
  static getDerivedStateFromError(error: Error): Partial<SecurityBoundaryState> {
          ...
          <CardContent>
            <div className="space-y-4">
              <p className="text-sm text-red-700 dark:text-red-300">
                We detected an unexpected error that may indicate a security issue.
                For your protection, this component has been disabled.
              </p>
...
export function useSecurityMonitor(options: {
  ...
  const trackAction = React.useCallback((actionName: string, maxActions: number = 10, windowMs: number = 60000) => {
    const now = Date.now();
    ...
    if (recentActions.length >= maxActions) {
      ...
      onSecurityViolation?.('rate_limit', { actionName, count: recentActions.length });
      return false;
    }
